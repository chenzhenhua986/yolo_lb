...
void forward_region_layer(const layer l, network net)
{
    ...
    *(l.cost) = 0;
        ...
        for (j = 0; j < l.h; ++j) {
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) {
                    ...
                    if(*(net.seen) > 0){
                        box truth = {0};
                        truth.x = (i + pred.x)/l.w;
                        truth.y = (j + pred.y)/l.h;
	...
	float* loss_weight;
        loss_weight = calloc(200, sizeof(float));
        int* box_indices;
        box_indices = calloc(200, sizeof(int));
        float total_delta = 0; 
	...
	int boost_num = 0; 
	    ...
	    loss_weight[t] = iou;
	    box_indices[t] = box_index;	    
	    total_weight += pow(1-iou, 1.5);
	    if(iou <= 0.5){
		boost_num++;
	    }	    
	    total_delta += fabsf(l.delta[box_index + 0*(l.w*l.h)]) + fabsf(l.delta[box_index + 1*(l.w*l.h)]) + fabsf(l.delta[box_index + 2*(l.w*l.h)]) + fabsf(l.delta[box_index + 3*(l.w*l.h)]);
	    ...
	    int p, q;
	    for(q=0; q<200; ++q){
                if(!loss_weight[q]) continue;
	            if(loss_weight[q] > .03 && loss_weight[q] < .5){
	                for(p=0; p<2; ++p){
	                    l.delta[box_indices[q] + p*(l.w*l.h)] *= pow((float)t/(boost_num+1), .5);
			}
	            }else if(loss_weight[q] > .8){
	                for(p=0; p<2; ++p){
	                    l.delta[box_indices[q] + p*(l.w*l.h)] /= pow((float)t/(boost_num+1), .5);
			}
		    }
